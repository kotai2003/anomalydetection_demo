...
def render(data: AppData) -> None:
    st.info(
        ...
        # 太字の閉じ ** を約物（）」。など）の直後に置かないこと。CommonMark の
        # right-flanking 条件を満たさず、アスタリスクがそのまま表示される。
    ...
    # この閾値が 0.5 前後に出ることがあり、「異常スコアは確率だから 0.5 が境目」
    # という現場でよくある誤解を呼びやすい。閾値は数学が決めるものではない、が
    # このアプリの主張なので、ここは明示的に否定しておく。
    st.caption(
        "※ この閾値が 0.5 前後に出ることがあるが、"
        "**0.5 という決まった値があるわけではない**。異常スコアは確率ではないので、"
        "0.5 が境目という約束事は存在しない。この値は良品と欠陥の分布の位置・"
        "ばらつきと、欠陥割合で決まる — 上の「OK:NG 比」を動かせば目の前で動く"
        "（**欠陥が少ないほど右へ寄る**）。"
    )
    st.caption(
    ...
    st.caption(
        ...
        "外の点が外れ値。つまり**箱ひげ図の数字は {n_repeat} 回ぶんの要約**であって、"
